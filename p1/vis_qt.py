...
def img_qt(x:Tensor, qt:int=4) -> Tensor:
  nq = 256 // qt
  x = denormalize(x)
  vmin, vmax = x.min(), x.max()
  x = (x - vmin) / (vmax - vmin)
...
for x, y, z_get in tqdm(datatset):
  ...
  x_qt = img_qt(x, qt=QT) if QT else x
  z = snake_reshape_norm_padding(x.unsqueeze(0), rev=True)   # truth
  if QT:
    z_qt = snake_reshape_norm_padding(x_qt.unsqueeze(0), rev=True)   # approx truth
    fid = get_fidelity(z_qt, z).item()
    fid_list.append(fid)
    print('fid_approx:', fid)

  if 'vqc fid & gate cnt':
    z_vqc = amplitude_encode(z.flatten().numpy(), eps=EPS, gamma=GAMMA)
    if QT:
      z_vqc_qt = amplitude_encode(z_qt.flatten().numpy(), eps=EPS, gamma=GAMMA)
    else:
      z_vqc_qt = None
  ...
  if 'train':
    N_ITER = 100
    vqc_trained = []
    for circ in [z_vqc, z_vqc_qt]:
      if circ is None:
        vqc_trained.append(None)
        continue

      vqc = dq.QubitCircuit(circ.nqubit)
      for op in circ.operators:
        vqc.add(op)
      #lyr = dq.RyLayer(circ.nqubit, wires=list(range(circ.nqubit)), requires_grad=True)
      #lyr.init_para([0] * circ.nqubit)
      #vqc.add(lyr)

      s = time()
      optimizer = optim.Adam(vqc.parameters(), lr=0.02)
      for i in range(N_ITER):
        state = vqc().swapaxes(0, 1)     # [B=1, D=1024]
        ...
        if i % 10 == 0:
          print('fid:', -loss.item())
      t = time()
      state = vqc().swapaxes(0, 1)
      print(f'>> Fidelity:', get_fidelity(state, z).item(), f'(time: {t - s})')
      vqc_trained.append(vqc)
    z_vqc, z_vqc_qt = vqc_trained
  ...
  if 'vqc fid & gate cnt':
    ...
    if QT:
      z_vqc_qt_fid = get_fidelity(z_vqc_qt(), z).item()
      z_vqc_qt_gc  = count_gates(z_vqc_qt)
      print('z_vqc_qt_fid:', z_vqc_qt_fid)
      print('z_vqc_qt_gc:',  z_vqc_qt_gc)
      z_qt_enc_fid.append(z_vqc_qt_fid)
      z_qt_enc_gc .append(z_vqc_qt_gc)

  if not 'plot':
    if QT:
      plt.subplot(231) ; plt.title('x')              ; plt.imshow(img_to_01(x)                                           .permute([1, 2, 0]).numpy())
      plt.subplot(232) ; plt.title('z_snake')        ; plt.imshow(img_to_01(z                  .real.reshape(-1, 32, 32)).permute([1, 2, 0]).numpy())
      plt.subplot(233) ; plt.title('z_snake_vqc')    ; plt.imshow(img_to_01(z_vqc()   .detach().real.reshape(-1, 32, 32)).permute([1, 2, 0]).numpy())
      plt.subplot(234) ; plt.title('x_qt')           ; plt.imshow(img_to_01(x_qt)                                        .permute([1, 2, 0]).numpy())
      plt.subplot(235) ; plt.title('z_qt_snake')     ; plt.imshow(img_to_01(z_qt               .real.reshape(-1, 32, 32)).permute([1, 2, 0]).numpy())
      plt.subplot(236) ; plt.title('z_qt_snake_vqc') ; plt.imshow(img_to_01(z_vqc_qt().detach().real.reshape(-1, 32, 32)).permute([1, 2, 0]).numpy())
      plt.suptitle(f'({z_vqc_gc}; {z_vqc_fid}) --qt--> ({z_vqc_qt_gc}; {z_vqc_qt_fid})')
      plt.tight_layout()
      plt.show()
    else:
...
print('vqc fidelity(qt):',   mean(z_qt_enc_fid))
print('vqc gate_count:',     mean(z_enc_gc))
print('vqc gate_count(qt):', mean(z_qt_enc_gc))
